...
class Session:
    ...
    def __init__(self):
        ...
        self.game_exe = None  # install the user pointed at; overrides the built-ins
    ...
    def hunt_start(self, p):
        ...
        spec = versions.get(self.version)
        exe = Path(spec.game_dir).parent / "PokemonEmerald.exe"
        self.hunter.exe_path = exe if exe.exists() else Path(spec.exe)
    ...
    def set_game_path(self, p):
        ...
        The built-in specs hardcode the author's paths. This is what makes the
        app work on someone else's machine: pick the exe, and the pak, the AES
        key and the launch target all follow from it.
        """
        exe = p.get("exe")
        if not exe:
            self.game_exe = None
            return {"exe": None}
        spec = versions.from_exe(exe)        # raises if the layout is wrong
        self.game_exe = str(Path(exe))
        self.libs.pop("custom", None)        # re-read the pak from the new path
    ...
    def launch_game(self, p):
        ...
        import subprocess
        exe = p.get("exe") or self.game_exe
        if exe:
            exe = Path(exe)
        else:
            spec = versions.get(p.get("version", self.version))
            exe = Path(spec.exe) if spec.exe else None
        if not exe or not exe.exists():
            raise RuntimeError("game exe not found — pick it with Choose game")
    ...
    def library(self, version=None):
        ...
        if lib is None:
            ...
            lib = AssetLibrary(spec.id if spec.id != "custom" else self.version)
